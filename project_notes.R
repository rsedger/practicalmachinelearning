...
# gbm with caret and all predictors
inTrain2 <- createDataPartition(y=whole_train$classe, p=0.7, list=FALSE)
train_gbmFit2 <- whole_train[inTrain2,]
validation_gbmFit2 <- whole_train[-inTrain2,]
...
gbmFit2 <- train(classe ~ ., data = train_gbmFit2,
                 method = "gbm",
                 trControl = fitControl,
                 ## This last option is actually one
                 ## for gbm() that passes through
...
#No pre-processing
#Resampling: Cross-Validated (10 fold, repeated 10 times) 
#Summary of sample sizes: 252, 252, 253, 252, 252, 252, ... 
#Resampling results across tuning parameters:
#        
#        interaction.depth  n.trees  Accuracy   Kappa    
#        1                   50      0.9932740  0.9915038
#        1                  100      0.9929169  0.9910529
#        1                  150      0.9929169  0.9910529
#        2                   50      0.9936189  0.9919357
#        2                  100      0.9936189  0.9919357
#        2                  150      0.9936189  0.9919357
#        3                   50      0.9936189  0.9919357
#        3                  100      0.9936189  0.9919357
#        3                  150      0.9936189  0.9919357
#
#Tuning parameter 'shrinkage' was held constant at a value of 0.1
#Tuning
#parameter 'n.minobsinnode' was held constant at a value of 10
...
#There were 6952 predictors of which 12 had non-zero influence.

summary(gbmFit2)
# produces way too much data to record - basically says that all the statistical
# variables had zero influence
...
#1    2 0.9923561 0.9903294 0.003055910 0.003868288
#2   27 0.9918469 0.9896859 0.001017842 0.001288726
#3   52 0.9863151 0.9826851 0.002441540 0.003090493

rfFit2$results
#  mtry  Accuracy     Kappa  AccuracySD     KappaSD
#1    2 0.9919201 0.9897781 0.002553158 0.003231333
#2   27 0.9922114 0.9901474 0.002474417 0.003130259
#3   52 0.9860244 0.9823193 0.003921172 0.004960641

predict(rfFit1, newdata = testing) # add for individual probabilities: type = "prob"
# [1] B A B A A E D B A A B C B A E E A B B B
# Levels: A B C D E

# Notice how sensitive rf is to choice of seed - mtry (number of randomly selected
# parameters) goes from 2 (seed = 1234) to 27 (seed = 17). Since the predictors are
# randomly selected we get a different set of them in each Importance plot. OOB error
# goes from 0.68% to 0.71%. 
set.seed(17)
rfFit2 <- train(classe~., method = "rf", data=training, trControl = trainControl(method = "cv"), importance=TRUE)
...
#No pre-processing
#Resampling: Cross-Validated (10 fold) 
#Summary of sample sizes: 12363, 12363, 12363, 12364, 12365, 12362, ... 
#Resampling results across tuning parameters:
#        
#        mtry  Accuracy   Kappa    
#        2    0.9919201  0.9897781
#       27    0.9922114  0.9901474
#       52    0.9860244  0.9823193
#
#Accuracy was used to select the optimal model using  the largest value.
...
results <- resamples(list(RF1seed1234=rfFit1, RF2seed17=rfFit2))
dotplot(results)
